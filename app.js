...
const MUSIC_DIRECTOR_WORLDS={
  ...
  'mountain-blood':{labels:['RIDGE WIND','STRING CALL','WOOD ANSWER','STONE SILENCE'],call:{sample:'mountainString',gain:.031,filterType:'lowpass',frequency:3300,rate:.94},answer:{sample:'wood',gain:.026,filterType:'lowpass',frequency:1850,rate:.88}},
...
const APP_VERSION=document.querySelector('meta[name="app-version"]')?.content||'6.7';
...
if('serviceWorker'in navigator){
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('./sw.js',{updateViaCache:'none'});$('#coreVersionLabel')&&($('#coreVersionLabel').textContent=`${APP_VERSION} WORLD MUSIC DIRECTOR`);reg.update().catch(()=>{});
