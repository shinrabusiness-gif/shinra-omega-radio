...
function getLayerScale(worldIndex,i){const scales=[[.17,.2,.12],[.15,.14,.11],[.12,.16,.11],[.105,.095,.055,.09],[.11,.055,.035],[.12,.09,.065],[.095,.145,.075,.082]];return (scales[worldIndex]||[.11,.055,.035])[i]||.05}
...
const MUSIC_DIRECTOR_WORLDS={
  'black-sun':{labels:['ASH BREATH','IRON CALL','CORONA ANSWER','DEAD AIR'],call:{sample:'metalResonance',gain:.031,filterType:'lowpass',frequency:2200,rate:.88},answer:{sample:'radio',gain:.009,filterType:'bandpass',frequency:1250,q:.8,rate:.64}},
  aquatoc:{labels:['DEEP CURRENT','SONAR CALL','PRESSURE ANSWER','STILL WATER'],call:{sample:'bubbleField',gain:.025,filterType:'bandpass',frequency:1700,q:.48,rate:1.05},answer:{sample:'pressureCrack',gain:.023,filterType:'lowpass',frequency:1050,rate:.91}},
  'crimson-rain':{labels:['RAIN VEIL','NEON CALL','CITY ANSWER','EMPTY STREET'],call:{sample:'neonHum',gain:.011,filterType:'bandpass',frequency:840,q:.92,rate:.98},answer:{sample:'cityDistant',gain:.019,filterType:'lowpass',frequency:2800,rate:.94}},
  'mountain-blood':{labels:['HIGH AIR','STRING PRAYER','SUMMIT BELL','STONE SILENCE'],call:{sample:'summitString',gain:.027,filterType:'lowpass',frequency:3900,rate:.94,layerIndex:1},answer:{sample:'summitBell',gain:.018,filterType:'highpass',frequency:340,rate:.92,layerIndex:3}},
  'starless-void':{labels:['COLD DRIFT','ION CALL','BEACON ANSWER','SIGNAL VOID'],call:{sample:'ionPulse',gain:.026,filterType:'lowpass',frequency:1180,rate:.82},answer:{sample:'radio',gain:.0075,filterType:'highpass',frequency:1650,rate:.43}},
  'celestial-veil':{labels:['CHOIR BREATH','HALO CALL','BELL ANSWER','MERCY SILENCE'],call:{sample:'glassChime',gain:.023,filterType:'highpass',frequency:560,rate:1.02},answer:{sample:'bell',gain:.021,filterType:'highpass',frequency:460,rate:.94}},
...
function musicDirectorCueGain(){if(listeningMode==='sleep')return .66;if(listeningMode==='focus')return .82;if(listeningMode==='cinematic')return 1.16;return 1}
...
function playMusicDirectorCue(stageIndex){if(stageIndex!==1&&stageIndex!==2)return;if(scannerActive||ritualActive)return;const worldIndex=current,def=directorDefinition(worldIndex),answer=stageIndex===2;pulseDirectorVisual();if(worldIndex===6){if(answer)playDirectorKoto();else if(typeof sunrootReedCue==='function')sunrootReedCue();else playDirectorFallback(worldIndex,false);return}const cue=answer?def.answer:def.call;if(cue&&settings.hybrid!==false&&sampleBuffers[cue.sample])playHybridSample(cue.sample,{...cue,gain:(cue.gain||.02)*musicDirectorCueGain(),pan:answer?.24:-.24});else playDirectorFallback(worldIndex,answer)}
...
async function startHybridLayer(worldIndex=current){
  ...
  else if(worldIndex===3){playHybridSample('summitWind',{loop:true,gain:.019*evolutionBoost,filterType:'highpass',frequency:160,rate:.94,pan:-.08,layerIndex:0});playHybridSample('summitDrone',{loop:true,gain:.012*evolutionBoost,filterType:'lowpass',frequency:2200,rate:.98,pan:.08,layerIndex:3});scheduleHybridOneShot('summitString',8,14,{gain:.024*evolutionBoost,filterType:'lowpass',frequency:3900,randomPan:.38,rateVariation:.08,firstDelay:3.2,layerIndex:1});scheduleHybridOneShot('summitBell',13,22,{gain:.015,filterType:'highpass',frequency:330,randomPan:.42,rateVariation:.07,firstDelay:7.5,layerIndex:3})}
...
const APP_VERSION=document.querySelector('meta[name="app-version"]')?.content||'6.8';
...
if('serviceWorker'in navigator){
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('./sw.js',{updateViaCache:'none'});$('#coreVersionLabel')&&($('#coreVersionLabel').textContent=`${APP_VERSION} MOUNTAIN ASCENT REWORK`);reg.update().catch(()=>{});
