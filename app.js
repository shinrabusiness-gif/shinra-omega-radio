...
if(localStorage.srAudioGeneration!=='8'){state.layers=[.70,.48,.34,.30];state.volume=Math.max(.60,Math.min(.70,state.volume||.66));localStorage.srLayers=JSON.stringify(state.layers);localStorage.srVol=String(state.volume);localStorage.srAudioGeneration='8'}
const APP_CACHE='shinra-omega-serenity-rebuild-8.0.0';
...
const HYBRID_SAMPLE_FILES={
  forest:'serenity-forest-breath.ogg',rain:'serenity-rain-veil.ogg',ocean:'serenity-water-breath.ogg',radio:'serenity-space-breath.ogg',wood:'serenity-reed-pluck.ogg',bell:'serenity-halo-bell.ogg',bamboo:'serenity-reed-pluck.ogg',
  ashWind:'serenity-ash-breath.ogg',metalResonance:'serenity-bowl-bloom.ogg',bubbleField:'serenity-water-drop.ogg',pressureCrack:'serenity-water-drop.ogg',
  neonHum:'serenity-night-glow.ogg',cityDistant:'serenity-rain-veil.ogg',mountainString:'serenity-string-prayer.ogg',emberFire:'serenity-ash-breath.ogg',summitWind:'serenity-ridge-air.ogg',summitString:'serenity-string-prayer.ogg',summitDrone:'serenity-mountain-blood-foundation.ogg',summitBell:'serenity-mountain-blood-bloom.ogg',
  cosmicStatic:'serenity-space-breath.ogg',ionPulse:'serenity-ion-bloom.ogg',choirAir:'serenity-choir-breath.ogg',glassChime:'serenity-halo-bell.ogg',cedarLeaves:'serenity-forest-breath.ogg',koto:'serenity-reed-pluck.ogg',
  omegaBlackSunBed:'serenity-black-sun-foundation.ogg',omegaBlackSunMotion:'serenity-black-sun-atmosphere.ogg',omegaBlackSunMelody:'serenity-black-sun-melody.ogg',omegaBlackSunSignature:'serenity-black-sun-bloom.ogg',
  omegaAquatocBed:'serenity-aquatoc-foundation.ogg',omegaAquatocMotion:'serenity-aquatoc-atmosphere.ogg',omegaAquatocMelody:'serenity-aquatoc-melody.ogg',omegaAquatocSignature:'serenity-aquatoc-bloom.ogg',
  omegaCrimsonRainBed:'serenity-crimson-rain-foundation.ogg',omegaCrimsonRainMotion:'serenity-crimson-rain-atmosphere.ogg',omegaCrimsonRainMelody:'serenity-crimson-rain-melody.ogg',omegaCrimsonRainSignature:'serenity-crimson-rain-bloom.ogg',
  omegaMountainBloodBed:'serenity-mountain-blood-foundation.ogg',omegaMountainBloodMotion:'serenity-mountain-blood-atmosphere.ogg',omegaMountainBloodMelody:'serenity-mountain-blood-melody.ogg',omegaMountainBloodSignature:'serenity-mountain-blood-bloom.ogg',
  omegaStarlessVoidBed:'serenity-starless-void-foundation.ogg',omegaStarlessVoidMotion:'serenity-starless-void-atmosphere.ogg',omegaStarlessVoidMelody:'serenity-starless-void-melody.ogg',omegaStarlessVoidSignature:'serenity-starless-void-bloom.ogg',
  omegaCelestialVeilBed:'serenity-celestial-veil-foundation.ogg',omegaCelestialVeilMotion:'serenity-celestial-veil-atmosphere.ogg',omegaCelestialVeilMelody:'serenity-celestial-veil-melody.ogg',omegaCelestialVeilSignature:'serenity-celestial-veil-bloom.ogg',
  omegaSunrootValeBed:'serenity-sunroot-vale-foundation.ogg',omegaSunrootValeMotion:'serenity-sunroot-vale-atmosphere.ogg',omegaSunrootValeMelody:'serenity-sunroot-vale-melody.ogg',omegaSunrootValeSignature:'serenity-sunroot-vale-bloom.ogg'
};
...
async function loadSampleBank(){
  ...
  updateHybridStatus('LOADING SERENITY BANK');
  const byUrl={};
  sampleLoadPromise=Promise.all(Object.entries(HYBRID_SAMPLE_FILES).map(async([key,url])=>{
    try{if(!byUrl[url])byUrl[url]=fetch(url).then(response=>{if(!response.ok)throw new Error(`${response.status}`);return response.arrayBuffer()}).then(data=>audioCtx.decodeAudioData(data));sampleBuffers[key]=await byUrl[url]}catch(error){console.warn('Serenity sample unavailable',key,error)}
  })).then(()=>{const count=Object.keys(sampleBuffers).length;updateHybridStatus(count?`SERENITY 48K · ${count}/${HYBRID_SAMPLE_TOTAL}`:'AUDIO BANK UNAVAILABLE');return sampleBuffers});
...
const OMEGA_AUDIO_WORLDS={
  'black-sun':{bed:'omegaBlackSunBed',motion:'omegaBlackSunMotion',melody:'omegaBlackSunMelody',signature:'omegaBlackSunSignature',bedGain:1.18,motionGain:.92,melodyGain:.88,signatureGain:1.12,bedFilter:11800,motionFilter:9800,melodyFilter:9200},
  aquatoc:{bed:'omegaAquatocBed',motion:'omegaAquatocMotion',melody:'omegaAquatocMelody',signature:'omegaAquatocSignature',bedGain:1.16,motionGain:.94,melodyGain:.88,signatureGain:1.08,bedFilter:10600,motionFilter:8400,melodyFilter:8800},
  'crimson-rain':{bed:'omegaCrimsonRainBed',motion:'omegaCrimsonRainMotion',melody:'omegaCrimsonRainMelody',signature:'omegaCrimsonRainSignature',bedGain:1.10,motionGain:.86,melodyGain:.82,signatureGain:1.02,bedFilter:12400,motionFilter:10500,melodyFilter:9200},
  'mountain-blood':{bed:'omegaMountainBloodBed',motion:'omegaMountainBloodMotion',melody:'omegaMountainBloodMelody',signature:'omegaMountainBloodSignature',bedGain:1.12,motionGain:.88,melodyGain:.76,signatureGain:1.02,bedFilter:11900,motionFilter:10400,melodyFilter:8800},
  'starless-void':{bed:'omegaStarlessVoidBed',motion:'omegaStarlessVoidMotion',melody:'omegaStarlessVoidMelody',signature:'omegaStarlessVoidSignature',bedGain:1.18,motionGain:.82,melodyGain:.78,signatureGain:1.08,bedFilter:10200,motionFilter:7800,melodyFilter:8400},
  'celestial-veil':{bed:'omegaCelestialVeilBed',motion:'omegaCelestialVeilMotion',melody:'omegaCelestialVeilMelody',signature:'omegaCelestialVeilSignature',bedGain:1.06,motionGain:.82,melodyGain:.74,signatureGain:.98,bedFilter:13200,motionFilter:11600,melodyFilter:11000},
  'sunroot-vale':{bed:'omegaSunrootValeBed',motion:'omegaSunrootValeMotion',melody:'omegaSunrootValeMelody',signature:'omegaSunrootValeSignature',bedGain:1.12,motionGain:.88,melodyGain:.82,signatureGain:1.04,bedFilter:12200,motionFilter:10400,melodyFilter:9400}
};
function omegaWorldAudio(index=current){return OMEGA_AUDIO_WORLDS[WORLDS[index]?.id]||OMEGA_AUDIO_WORLDS['black-sun']}
function playOmegaSignature(index=current,scale=1,delay=0){const cfg=omegaWorldAudio(index);if(!cfg||!sampleBuffers[cfg.signature])return null;return playHybridSample(cfg.signature,{gain:cfg.signatureGain*scale*musicDirectorCueGain(),filterType:'lowpass',frequency:index===5?13200:10400,pan:0,rate:1,delay,fadeIn:.35,role:'bloom'})}
function startOmegaWorldStems(worldIndex=current,evolutionBoost=1){const cfg=omegaWorldAudio(worldIndex);if(!cfg)return;const mode=listeningMode==='sleep'?.64:listeningMode==='focus'?.82:listeningMode==='cinematic'?1.08:1;playHybridSample(cfg.bed,{loop:true,gain:cfg.bedGain*evolutionBoost*mode,filterType:'lowpass',frequency:cfg.bedFilter,q:.2,rate:1,pan:0,fadeIn:8,randomOffset:false,layerIndex:0,role:'foundation'});playHybridSample(cfg.motion,{loop:true,gain:cfg.motionGain*evolutionBoost*mode,filterType:'lowpass',frequency:cfg.motionFilter,q:.2,rate:1,pan:0,fadeIn:10,randomOffset:true,layerIndex:1,role:'atmosphere'});playHybridSample(cfg.melody,{loop:true,gain:cfg.melodyGain*evolutionBoost*mode,filterType:'lowpass',frequency:cfg.melodyFilter,q:.18,rate:1,pan:0,fadeIn:13,randomOffset:true,layerIndex:2,role:'melody'});if(evolutionPhase===0){const generation=hybridGeneration;hybridTimers.push(setTimeout(()=>{if(generation===hybridGeneration&&playing&&current===worldIndex)playOmegaSignature(worldIndex,.26)},6200))}}
...
async function startHybridLayer(worldIndex=current){
  stopHybridLayer();if(settings.hybrid===false){updateHybridStatus('SERENITY ENGINE PAUSED');return}
...
const APP_VERSION=document.querySelector('meta[name="app-version"]')?.content||'8.0.0';
...
// ===== OMEGA SIGNAL NEXUS 8.0.0 — SERENITY REBUILD AUDIO ENGINE =====
(()=>{
