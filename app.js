...
const LISTENING_MODES={balanced:{label:'VELVET',audio:'SILK MASTER',events:'CONTROLLED',master:.88,eq:[.4,-.5,-1.2],layers:[1,.94,.86,.9]},focus:{label:'DEEP FOCUS',audio:'CENTERED',events:'QUIET',master:.8,eq:[.2,-.7,-2.1],layers:[1.02,.78,.66,.7]},sleep:{label:'SLEEP',audio:'SOFTENED',events:'MUTED',master:.66,eq:[.2,-1.2,-4.5],layers:[1.02,.56,.42,.46]},cinematic:{label:'CINEMATIC',audio:'EXPANDED',events:'HEIGHTENED',master:.92,eq:[1.1,-.25,-.55],layers:[1.08,1.02,1.04,1.02]}};
...
const APP_CACHE='shinra-omega-silk-master-7.1.0';
...
const HYBRID_SAMPLE_FILES={
  ...
  neonHum:'sample-neon-hum.wav',cityDistant:'sample-city-distant.wav',mountainString:'sample-mountain-string.wav',emberFire:'sample-ember-fire.wav',summitWind:'sample-summit-wind.wav',summitString:'sample-summit-string.wav',summitDrone:'sample-summit-drone.wav',summitBell:'sample-summit-bell.wav',
  ...
  omegaBlackSunBed:'omega-black-sun-bed.ogg',omegaBlackSunMotion:'omega-black-sun-motion.ogg',omegaBlackSunSignature:'omega-black-sun-signature.ogg',
  omegaAquatocBed:'omega-aquatoc-bed.ogg',omegaAquatocMotion:'omega-aquatoc-motion.ogg',omegaAquatocSignature:'omega-aquatoc-signature.ogg',
  omegaCrimsonRainBed:'omega-crimson-rain-bed.ogg',omegaCrimsonRainMotion:'omega-crimson-rain-motion.ogg',omegaCrimsonRainSignature:'omega-crimson-rain-signature.ogg',
  omegaMountainBloodBed:'omega-mountain-blood-bed.ogg',omegaMountainBloodMotion:'omega-mountain-blood-motion.ogg',omegaMountainBloodSignature:'omega-mountain-blood-signature.ogg',
  omegaStarlessVoidBed:'omega-starless-void-bed.ogg',omegaStarlessVoidMotion:'omega-starless-void-motion.ogg',omegaStarlessVoidSignature:'omega-starless-void-signature.ogg',
  omegaCelestialVeilBed:'omega-celestial-veil-bed.ogg',omegaCelestialVeilMotion:'omega-celestial-veil-motion.ogg',omegaCelestialVeilSignature:'omega-celestial-veil-signature.ogg',
  omegaSunrootValeBed:'omega-sunroot-vale-bed.ogg',omegaSunrootValeMotion:'omega-sunroot-vale-motion.ogg',omegaSunrootValeSignature:'omega-sunroot-vale-signature.ogg'
};
...
async function loadSampleBank(){
  ...
  })).then(()=>{const count=Object.keys(sampleBuffers).length;updateHybridStatus(count?`SILK AUDIO · ${count}/${HYBRID_SAMPLE_TOTAL}`:'WEB AUDIO FALLBACK');return sampleBuffers});
...
const OMEGA_AUDIO_WORLDS={
  'black-sun':{bed:'omegaBlackSunBed',motion:'omegaBlackSunMotion',signature:'omegaBlackSunSignature',bedGain:.158,motionGain:.078,signatureGain:.155,bedRate:.985,motionRate:.94,bedFilter:5200,motionFilter:3000,bedType:'lowpass',motionType:'lowpass'},
  aquatoc:{bed:'omegaAquatocBed',motion:'omegaAquatocMotion',signature:'omegaAquatocSignature',bedGain:.154,motionGain:.082,signatureGain:.148,bedRate:.98,motionRate:.93,bedFilter:5000,motionFilter:5200,bedType:'lowpass',motionType:'bandpass',motionQ:.32},
  'crimson-rain':{bed:'omegaCrimsonRainBed',motion:'omegaCrimsonRainMotion',signature:'omegaCrimsonRainSignature',bedGain:.145,motionGain:.094,signatureGain:.142,bedRate:1,motionRate:1,bedFilter:10000,motionFilter:5800,bedType:'lowpass',motionType:'lowpass'},
  'mountain-blood':{bed:'omegaMountainBloodBed',motion:'omegaMountainBloodMotion',signature:'omegaMountainBloodSignature',bedGain:.162,motionGain:.102,signatureGain:.15,bedRate:.99,motionRate:.985,bedFilter:7200,motionFilter:5600,bedType:'lowpass',motionType:'lowpass'},
  'starless-void':{bed:'omegaStarlessVoidBed',motion:'omegaStarlessVoidMotion',signature:'omegaStarlessVoidSignature',bedGain:.156,motionGain:.074,signatureGain:.158,bedRate:.965,motionRate:.88,bedFilter:5400,motionFilter:6500,bedType:'lowpass',motionType:'bandpass',motionQ:.24},
  'celestial-veil':{bed:'omegaCelestialVeilBed',motion:'omegaCelestialVeilMotion',signature:'omegaCelestialVeilSignature',bedGain:.142,motionGain:.088,signatureGain:.136,bedRate:1,motionRate:.995,bedFilter:11000,motionFilter:7600,bedType:'lowpass',motionType:'lowpass'},
  'sunroot-vale':{bed:'omegaSunrootValeBed',motion:'omegaSunrootValeMotion',signature:'omegaSunrootValeSignature',bedGain:.148,motionGain:.094,signatureGain:.142,bedRate:.995,motionRate:1,bedFilter:9000,motionFilter:5200,bedType:'lowpass',motionType:'lowpass'}
};
function omegaWorldAudio(index=current){return OMEGA_AUDIO_WORLDS[WORLDS[index]?.id]||OMEGA_AUDIO_WORLDS['black-sun']}
function playOmegaSignature(index=current,scale=1,delay=0){const cfg=omegaWorldAudio(index);if(!cfg||!sampleBuffers[cfg.signature])return null;return playHybridSample(cfg.signature,{gain:cfg.signatureGain*scale*musicDirectorCueGain(),filterType:'lowpass',frequency:index===5?12000:7600,pan:(index%2?-.12:.12),rate:1+(evolutionPhase*.008),delay,fadeIn:.08,role:'signature'})}
function startOmegaWorldStems(worldIndex=current,evolutionBoost=1){const cfg=omegaWorldAudio(worldIndex);if(!cfg)return;const mode=listeningMode==='sleep'?.62:listeningMode==='focus'?.8:listeningMode==='cinematic'?1.02:.9;playHybridSample(cfg.bed,{loop:true,gain:cfg.bedGain*evolutionBoost*mode,filterType:cfg.bedType,frequency:cfg.bedFilter,q:.24,rate:cfg.bedRate,pan:0,fadeIn:5.2,randomOffset:false,role:'omega-bed'});playHybridSample(cfg.motion,{loop:true,gain:cfg.motionGain*evolutionBoost*mode,filterType:cfg.motionType,frequency:cfg.motionFilter,q:cfg.motionQ||.3,rate:cfg.motionRate,pan:worldIndex%2?.08:-.08,fadeIn:7.2,randomOffset:true,role:'omega-motion'});if(evolutionPhase===0){const generation=hybridGeneration;hybridTimers.push(setTimeout(()=>{if(generation===hybridGeneration&&playing&&current===worldIndex)playOmegaSignature(worldIndex,.4)},2900))}}

const MUSIC_DIRECTOR_STAGES=[
  {id:'breath',generic:'BREATH',mix:[.92,.66,.58,.52],min:9,max:14},
  {id:'call',generic:'CALL',mix:[.96,1.10,.72,.76],min:7,max:11},
  {id:'answer',generic:'ANSWER',mix:[.86,.82,1.10,1.14],min:7,max:11},
  {id:'silence',generic:'SILENCE',mix:[.72,.48,.46,.40],min:11,max:18}
];
const MUSIC_DIRECTOR_WORLDS={
  'black-sun':{labels:['ASH BREATH','IRON CALL','CORONA ANSWER','DEAD AIR'],call:{sample:'metalResonance',gain:.031,filterType:'lowpass',frequency:2200,rate:.88},answer:{sample:'omegaBlackSunSignature',gain:.088,filterType:'lowpass',frequency:7200,rate:1}},
  aquatoc:{labels:['DEEP CURRENT','SONAR CALL','PRESSURE ANSWER','STILL WATER'],call:{sample:'bubbleField',gain:.025,filterType:'bandpass',frequency:1700,q:.48,rate:1.05},answer:{sample:'omegaAquatocSignature',gain:.082,filterType:'lowpass',frequency:7600,rate:1}},
  'crimson-rain':{labels:['RAIN VEIL','NEON CALL','CITY ANSWER','EMPTY STREET'],call:{sample:'neonHum',gain:.011,filterType:'bandpass',frequency:840,q:.92,rate:.98},answer:{sample:'omegaCrimsonRainSignature',gain:.078,filterType:'lowpass',frequency:8200,rate:1}},
  'mountain-blood':{labels:['HIGH AIR','STRING PRAYER','SUMMIT BELL','STONE SILENCE'],call:{sample:'summitString',gain:.027,filterType:'lowpass',frequency:3900,rate:.94,layerIndex:1},answer:{sample:'omegaMountainBloodSignature',gain:.082,filterType:'lowpass',frequency:8200,rate:1}},
  'starless-void':{labels:['COLD DRIFT','ION CALL','BEACON ANSWER','SIGNAL VOID'],call:{sample:'ionPulse',gain:.026,filterType:'lowpass',frequency:1180,rate:.82},answer:{sample:'omegaStarlessVoidSignature',gain:.086,filterType:'lowpass',frequency:7000,rate:.96}},
  'celestial-veil':{labels:['CHOIR BREATH','HALO CALL','BELL ANSWER','MERCY SILENCE'],call:{sample:'glassChime',gain:.023,filterType:'highpass',frequency:560,rate:1.02},answer:{sample:'omegaCelestialVeilSignature',gain:.072,filterType:'lowpass',frequency:12000,rate:1}},
  'sunroot-vale':{labels:['FOREST BREATH','REED CALL','KOTO ANSWER','ROOTED SILENCE']}
};
...
function musicDirectorTimeFactor(){if(listeningMode==='sleep')return 1.42;if(listeningMode==='focus')return 1.18;if(listeningMode==='cinematic')return .82;return 1}
function musicDirectorCueGain(){if(listeningMode==='sleep')return .5;if(listeningMode==='focus')return .68;if(listeningMode==='cinematic')return .94;return .78}
...
function playMusicDirectorCue(stageIndex){if(stageIndex!==1&&stageIndex!==2)return;if(scannerActive||ritualActive)return;const worldIndex=current,def=directorDefinition(worldIndex),answer=stageIndex===2;pulseDirectorVisual();if(worldIndex===6){if(answer){playOmegaSignature(6,.82);setTimeout(()=>{if(playing&&current===6)playDirectorKoto()},620)}else if(typeof sunrootReedCue==='function')sunrootReedCue();else playDirectorFallback(worldIndex,false);return}const cue=answer?def.answer:def.call;if(cue&&settings.hybrid!==false&&sampleBuffers[cue.sample])playHybridSample(cue.sample,{...cue,gain:(cue.gain||.02)*musicDirectorCueGain(),pan:answer?.24:-.24});else playDirectorFallback(worldIndex,answer)}
...
async function startHybridLayer(worldIndex=current){
  stopHybridLayer();if(settings.hybrid===false){updateHybridStatus('PROCEDURAL AUDIO');return}
  const generation=hybridGeneration;await loadSampleBank();if(generation!==hybridGeneration||!playing||worldIndex!==current)return;const evolutionBoost=1+Math.max(0,evolutionPhase)*.075;
  startOmegaWorldStems(worldIndex,evolutionBoost);
  if(worldIndex===0){playHybridSample('ashWind',{loop:true,gain:.0075*evolutionBoost,filterType:'lowpass',frequency:760,rate:.82,pan:-.18,fadeIn:4.8,role:'detail'});}
  else if(worldIndex===1){playHybridSample('ocean',{loop:true,gain:.0085*evolutionBoost,filterType:'lowpass',frequency:1100,rate:.86,fadeIn:4.5,role:'detail'});}
  else if(worldIndex===2){playHybridSample('rain',{loop:true,gain:.009*evolutionBoost,filterType:'highpass',frequency:680,rate:1.01,fadeIn:3.8,role:'detail'});}
  else if(worldIndex===3){playHybridSample('summitWind',{loop:true,gain:.007*evolutionBoost,filterType:'highpass',frequency:190,rate:.91,pan:-.12,fadeIn:4.8,layerIndex:0,role:'detail'});}
  else if(worldIndex===4){playHybridSample('cosmicStatic',{loop:true,gain:.0055*evolutionBoost,filterType:'bandpass',frequency:2300,q:.32,rate:.68,pan:.14,fadeIn:5.5,role:'detail'});}
  else if(worldIndex===5){playHybridSample('choirAir',{loop:true,gain:.006*evolutionBoost,filterType:'highpass',frequency:520,rate:.88,pan:-.1,fadeIn:5.8,role:'detail'});}
  else if(worldIndex===6){playHybridSample('cedarLeaves',{loop:true,gain:.0058*evolutionBoost,filterType:'bandpass',frequency:2600,q:.28,rate:.95,pan:-.14,fadeIn:4.8,role:'detail'});if(settings.musicDirector===false)scheduleSunlitKoto(evolutionBoost*.9)}
}


function ensureAudio(){if(audioCtx)return;audioCtx=new (window.AudioContext||window.webkitAudioContext)({latencyHint:'playback'});master=audioCtx.createGain();masterDcBlock=audioCtx.createBiquadFilter();masterComfortPresence=audioCtx.createBiquadFilter();masterComfortAir=audioCtx.createBiquadFilter();masterSafetyLowpass=audioCtx.createBiquadFilter();masterCompressor=audioCtx.createDynamicsCompressor();masterLimiter=audioCtx.createDynamicsCompressor();analyser=audioCtx.createAnalyser();analyser.fftSize=512;analyser.smoothingTimeConstant=.86;filters=[audioCtx.createBiquadFilter(),audioCtx.createBiquadFilter(),audioCtx.createBiquadFilter()];filters[0].type='lowshelf';filters[0].frequency.value=145;filters[1].type='peaking';filters[1].frequency.value=1050;filters[1].Q.value=.72;filters[2].type='highshelf';filters[2].frequency.value=4300;masterDcBlock.type='highpass';masterDcBlock.frequency.value=24;masterDcBlock.Q.value=.55;masterComfortPresence.type='peaking';masterComfortPresence.frequency.value=2800;masterComfortPresence.Q.value=.78;masterComfortPresence.gain.value=-1.5;masterComfortAir.type='highshelf';masterComfortAir.frequency.value=7600;masterComfortAir.gain.value=-.85;masterSafetyLowpass.type='lowpass';masterSafetyLowpass.frequency.value=15500;masterSafetyLowpass.Q.value=.55;masterCompressor.threshold.value=-16;masterCompressor.knee.value=24;masterCompressor.ratio.value=1.65;masterCompressor.attack.value=.05;masterCompressor.release.value=.65;masterLimiter.threshold.value=-3.2;masterLimiter.knee.value=5;masterLimiter.ratio.value=8;masterLimiter.attack.value=.008;masterLimiter.release.value=.24;filters[0].connect(filters[1]).connect(filters[2]).connect(masterComfortPresence).connect(masterComfortAir).connect(masterSafetyLowpass).connect(analyser).connect(master).connect(masterDcBlock).connect(masterCompressor).connect(masterLimiter).connect(audioCtx.destination);master.gain.value=effectiveVolume();applyEq();applyPerformanceProfile('audio context');drawVisualizer();}
...
async function startFusionAudio(){stopFusionAudio();ensureFusionSecondary();if(!fusionActive||!playing||fusionSecondary===current||!audioCtx)return;const generation=fusionGeneration,w=WORLDS[fusionSecondary],t=audioCtx.currentTime;fusionBus=audioCtx.createGain();fusionBus.gain.setValueAtTime(.0001,t);fusionBus.gain.linearRampToValueAtTime(fusionMix*.56,t+2.4);fusionBus.connect(filters[0]);fusionTrackNode(fusionBus);
  const base=Math.max(32,Math.min(180,w.osc?.[0]||82.4)),o1=audioCtx.createOscillator(),o2=audioCtx.createOscillator(),g1=audioCtx.createGain(),g2=audioCtx.createGain(),p1=audioCtx.createStereoPanner(),p2=audioCtx.createStereoPanner(),lfo=audioCtx.createOscillator(),depth=audioCtx.createGain();o1.type='sine';o2.type=fusionSecondary===2||fusionSecondary===3?'triangle':'sine';o1.frequency.value=base;o2.frequency.value=base*1.5;o1.detune.value=-7;o2.detune.value=6;g1.gain.value=.16;g2.gain.value=.072;p1.pan.value=-.32;p2.pan.value=.38;lfo.frequency.value=.021+fusionSecondary*.003;depth.gain.value=2.5;lfo.connect(depth).connect(o2.detune);o1.connect(g1).connect(p1).connect(fusionBus);o2.connect(g2).connect(p2).connect(fusionBus);o1.start();o2.start();lfo.start();fusionSources.push(o1,o2,lfo);fusionTrackNode(g1,g2,p1,p2,depth);
  const noise=makeNoise(fusionSecondary===2?'white':fusionSecondary===5?'pink':'brown',6),nf=audioCtx.createBiquadFilter(),ng=audioCtx.createGain(),np=audioCtx.createStereoPanner();nf.type=fusionSecondary===4?'highpass':fusionSecondary===2?'bandpass':'lowpass';nf.frequency.value=[520,1250,2600,900,1900,3300,1800][fusionSecondary]||1200;nf.Q.value=fusionSecondary===2?1.1:.45;ng.gain.value=.052;np.pan.value=.18;noise.connect(nf).connect(ng).connect(np).connect(fusionBus);noise.start();fusionSources.push(noise);fusionTrackNode(nf,ng,np);
  await loadSampleBank();if(generation!==fusionGeneration||!fusionActive||!playing)return;
  startFusionPairVoice(fusionPairKey(),generation);
  if(fusionSecondary===0){playFusionSample('radio',{loop:true,gain:.013,frequency:980,type:'bandpass',rate:.66,pan:.2});playFusionSample('forest',{loop:true,gain:.0065,frequency:380,rate:.55,pan:-.2})}
  else if(fusionSecondary===1){playFusionSample('ocean',{loop:true,gain:.031,frequency:1500,rate:.86});playFusionSample('radio',{loop:true,gain:.006,frequency:620,type:'bandpass',rate:.5,pan:.28})}
  else if(fusionSecondary===2){playFusionSample('rain',{loop:true,gain:.03,frequency:650,type:'highpass',rate:1.04});playFusionSample('radio',{loop:true,gain:.0065,frequency:1700,type:'bandpass',rate:.8,pan:-.25})}
  else if(fusionSecondary===3){playFusionSample('forest',{loop:true,gain:.024,frequency:1200,rate:.7,pan:-.14});scheduleFusionSample('wood',6,10,{gain:.038,frequency:1600,firstDelay:2.2})}
  else if(fusionSecondary===4){playFusionSample('radio',{loop:true,gain:.01,frequency:1800,type:'highpass',rate:.44,pan:.2});playFusionSample('ocean',{loop:true,gain:.0055,frequency:105,rate:.4,pan:-.25})}
  else if(fusionSecondary===5){playFusionSample('forest',{loop:true,gain:.0062,frequency:2700,type:'highpass',rate:.8,pan:-.2});scheduleFusionSample('bell',7,12,{gain:.03,frequency:500,type:'highpass',firstDelay:2.6})}
  else if(fusionSecondary===6){playFusionSample('forest',{loop:true,gain:.024,frequency:2200,rate:.94,pan:.08});scheduleFusionSample('bamboo',9,15,{gain:.035,frequency:1250,type:'bandpass',firstDelay:3.2});scheduleFusionSample('wood',7,12,{gain:.03,frequency:1550,firstDelay:5})}
}
...
const APP_VERSION=document.querySelector('meta[name="app-version"]')?.content||'7.1.0';
...
// ===== OMEGA SIGNAL NEXUS 7.1.0 — SILK MASTER AUDIO ENGINE =====
(()=>{
